...
const redirectMealRecipes = (mealRecipes, history, setRecipes) => {
  ...
  if (mealRecipes && mealRecipes.length === 1) return history.push(`/comidas/${mealRecipes[0].idMeal}`);
...
const redirectDrinkRecipes = (drinkRecipes, history, setRecipes) => {
  ...
  if (drinkRecipes && drinkRecipes.length === 1) return history.push(`/bebidas/${drinkRecipes[0].idDrink}`);
